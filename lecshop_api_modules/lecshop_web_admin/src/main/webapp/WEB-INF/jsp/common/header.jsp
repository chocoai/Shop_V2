...
<header class="header white-bg">
    ...
    <a href="index.htm" class="logo"><img src="http://lecshop.b0.upaiyun.com/1496280394557.jpg" style="max-height: 35px"> </a>
    <ul class="nav_ace">
        <c:forEach items="${adminMenu}" var="firstMenu">
                            ...
                            <c:when test="${secondMenu.childMenu!=null}">
                                <c:if test="${i.index==0 }">
                                    <c:forEach items="${secondMenu.childMenu}" var="thirdMenu" varStatus="j">
                                        <c:if test="${j.index==0 }">
                                            <c:set var="url"
                                                   value="${thirdMenu.url}?firstMenus=${firstMenu.authorityId}&secondMenus=${secondMenu.authorityId}&thirdMenus=${thirdMenu.authorityId}"></c:set>
                                        </c:if>
                                    </c:forEach>
                                </c:if>
                            </c:when>
            ...
            <li>
                <a nid="${firstMenu.authorityId}"
                   <c:if test="${firstMenu.authorityId==firstMenus}">class="cur"</c:if> href="${url}"> ${firstMenu.name}</a>
            </li>
        </c:forEach>
    </ul>
    <div class="top-nav">
        <ul class="nav pull-right top-menu">
            <li class="dropdown"><a data-toggle="dropdown" class="dropdown-toggle" href="#"> <img alt="" src="<%=basePath %>/res/img/avatar1_small.jpg"> <span
                    class="username">${ADMIN_LOGIN_SESSION_KEY.username}</span>
                <b class="caret"></b> </a>
...
<script type="text/javascript">
    $(function () {
        LSFetch({
            url: '<%=basePath%>/querybaseinfosetunauth.htm',

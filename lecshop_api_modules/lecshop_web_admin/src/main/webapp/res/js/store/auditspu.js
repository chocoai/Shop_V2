...
function loadSkuBodys() {
    ...
    for (var i = 0; i < updateSpu.skus.length; i++) {
        ...
        if (sku.status == "0") {
            ...
            auditBtn = '<button type="button" class="btn btn-success btn-xs" onclick="toAudit(\''+ sku.id + '\')"><i class="icon-search"></i> 审核</button>';
        }
        ...
        skuHtml += '<td><input disabled unique="' + sku.unique + '" onchange="weightChange(this)" type="text" value="' + sku.weight + '" class="form-control sku_weight"></td>' +
            ...
            '<td> ' + auditBtn + ' <button type="button" onclick="toEditSku(' + sku.unique + ')" class="btn btn-primary btn-xs edit_btn"><i class="icon-pencil"></i> 查看详细</button> </td></tr>';
        skuHtmls += skuHtml;
    }
...
function audit() {
    // 通过标记
    var isPass = $("#audit").val();
    // 拒绝原因
    var reason = $("#reason").val();
    // 单品id
    var skuIds = getSkIds($("#auditType").val());
    // 通过
    if (isPass == 1) {
        LSFetch({
            url: basePath + '/auditpass.htm',
            data: {skuIds: skuIds},
            async: false,
            success: function () {
                window.location.href = basePath + "toauditspus.htm";
            }
        })
    } else {
        // 拒绝
        LSFetch({
            url: basePath + '/auditrefuse.htm',
            data: {skuIds: skuIds, reason: reason},
            async: false,
            success: function () {
                window.location.href = basePath + "toauditspus.htm";
            }
...
// 获得审核的单品id
function getSkIds(type) {
    var skuIds = new Array();
    // 全部单品
    if (type == 1) {
        for (var i = 0; i < updateSpu.skus.length; i++) {
            skuIds.push(updateSpu.skus[i].id);
        }
    } else {
        skuIds.push($("#auditSkuId").val())
    }

    return skuIds.toString();
}

...
function deleteSpu() {
    LSFetch({
        url: basePath + '/deletespu.htm?spuId=' + $("#deleteId").val(),
        success: function () {
...
function deleteAll(ids) {
    LSFetch({
        url: basePath + '/deletespus.htm?ids=' + ids,
        success: function () {
            $('#confirm').modal('hide');
            refreshDataTable();
        }
...
function batchup() {
    LSFetch({
        url: basePath + '/updateshelvesstatus.htm?ids=' + getSelectIds("id") + "&status=1",
        success: function () {
...
function batchdown() {
    LSFetch({
        url: basePath + '/updateshelvesstatus.htm?ids=' + getSelectIds("id") + "&status=0",
        success: function () {
...
// 跳转到添加商品页面
function toaddspu() {
    window.location.href = "toaddspu.htm";
}


// 跳转到修改商品页面
function toupdate(id) {
    window.location.href = "toupdatespu.htm?id="+id;
}

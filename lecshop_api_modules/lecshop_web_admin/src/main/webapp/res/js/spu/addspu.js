...
function loadTCate(obj, parentId, parentName) {
    ...
    LSFetch({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                var cate = data[i];
                html += '<li id="item' + cate.id + '" class="item"  onclick="clickThirdCate(this,' + cate.id + ',' + cate.typeId + ',\'' + cate.name + '\')">' +
                    '<div class="task-title item"> <span class="task-title-sp">' + cate.name + '</span>' +
                    '<div class="pull-right" style="display:none">' +
                    '</div>' +
                    '</div>' +
...
function clickThirdCate(obj, parentId, typeId, parentName) {
    $("#cateId").val(parentId);
    $("#typeId").val(typeId);
...
function loadType() {
    $("#attr tbody").html('');
    LSFetch({
        url: basePath + '/querytypedetail.htm?id=' + $("#typeId").val(),
...
function loadSpecs(specs) {
    ...
    // 查询规格信息
    LSFetch({
        url: basePath + '/queryspecsbyids.htm',
        ...
        success: function (specs) {
            ...
            for (var i = 0; i < specs.length; i++) {
                ...
                specHtml += '<div class="panel-heading clearfix"> <h4 class="panel-title fll">' + spec.name + ':</h4> <a href="javascript:;" class="add_sku flr">添加</a> </div>'
...
function loadSkuTableHead(allSpecNames) {
    ...
    html += '<th width="100"><span class="label_red">*</span>重量(克)</th><th width="165"><span class="label_red">*</span>价格(元)</th><th width="100"><span class="label_red">*</span>库存</th><th>操作</th>';
...
function addSpu() {
    ...
    spu.firstCateId = $("#fcateId").val();
    spu.secondCateId = $("#scateId").val();
    spu.thirdCateId = $("#cateId").val();
    spu.typeId = $("#typeId").val();

    ...
    LSFetch({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                html += '<label class="checkbox-inline"> <input  dis '+checkFlag+' type="checkbox" name="servicesupport" value="' + data[i].id + '">' + data[i].name + ' </label>';
            }
...
function loadCustomerLevel() {
    LSFetch({
        url: basePath + '/queryalllevels.htm',
        success: function (levels) {
            var html = '';
            for (var i = 0; i < levels.length; i++) {
                html += '<tr> <td>' + levels[i].name + '</td> <td><div class="form-group" style="margin:0"><input levelId="' + levels[i].id + '" id="level' + levels[i].id + '" type="text" value="" class="form-control isNoInteger"></div></td> </tr>';
            }
            $("#levels").html(html);
        }
    })
...
function loadSpuBaseInfo() {
    $("#catenames").html(updateSpu.firstCategory.name + '--' + updateSpu.secondCategory.name + '--' + updateSpu.thirdCategory.name);
...
function loadSkuHead() {
    ...
    html += '<th width="85"><span class="label_red">*</span>重量(克)</th><th width="165"><span class="label_red">*</span>价格(元)</th><th width="80"><span class="label_red">*</span>库存</th><th width="80">状态</th><th>操作</th>';
...
function loadSkuBodys() {
    ...
    for (var i = 0; i < updateSpu.skus.length; i++) {
        ...
        skuHtml += '<td><input unique="' + sku.unique + '" onchange="weightChange(this)" type="text" value="' + sku.weight + '" class="form-control sku_weight"></td>' +
            '<td> <input unique="' + sku.unique + '" onchange="priceChange(this)" type="text" value="' + sku.price + '" class="form-control fll sku_price" style="width:70px"> <button type="button" class="btn btn-primary vip_price" onclick="toEditCustomerPrice(' + sku.unique + ')" style="margin-left:5px">会员价</button> </td>' +
            ...
            '<td> <button type="button" onclick="toDelete(' + sku.unique + ')" class="btn btn-danger btn-xs del_btn"><i class="icon-trash"></i> 删除</button> <button type="button" onclick="toEditSku(' + sku.unique + ')" class="btn btn-primary btn-xs edit_btn"><i class="icon-pencil"></i> 编辑详细</button> </td></tr>';
        skuHtmls += skuHtml;
    }
    ...
    LSFetch({
        ...
        success: function (res) {
            if (res == 0) {
                ...
                window.location.href = "toqueryspus.htm";
            }
        }
    })
}
...
function addupdatesku() {
    ...
    skuHtml += '<td><input unique="' + uniqueId + '" onchange="weightChange(this)" type="text" value="" class="form-control sku_weight"></td>' +
        '<td> <input unique="' + uniqueId + '" onchange="priceChange(this)" type="text" value="' + sku.price + '" class="form-control fll sku_price" style="width:70px"> <button type="button" onclick="toEditCustomerPrice(' + uniqueId + ')" class="btn btn-primary vip_price" style="margin-left:5px">会员价</button> </td>' +
        '<td><input onchange="stockChange(this)" unique="' + uniqueId + '" type="text" value="" class="form-control sku_stock"></td>' +
        '<td> <button type="button" onclick="toDelete(' + uniqueId + ')" class="btn btn-danger btn-xs del_btn"><i class="icon-trash"></i> 删除</button> <button type="button" onclick="toEditSku(' + uniqueId + ')" class="btn btn-primary btn-xs edit_btn"><i class="icon-pencil"></i> 编辑详细</button> </td></tr>';
...
//编辑会员价格
function toEditCustomerPrice(id) {
    !$("#customerLevelForm")[0].reset();
    $("#customerPriceSkuId").val(id);

    var sku = findSku(id);

    for (var k = 0; k < sku.skuMemberPrices.length; k++) {
        $("#level" + sku.skuMemberPrices[k].memberLevelId).val(sku.skuMemberPrices[k].price);
    }

    $('#edit_vip_price').modal('show');
}

// 保存会员价格
function saveCustomerPrice() {
    if (!$("#customerLevelForm").valid()) {
        return;
    }

    var sku = findSku($("#customerPriceSkuId").val());

    sku.skuMemberPrices = new Array();
    $('#levels').find('tr').each(function () {
        if ($(this).find('input').val() != '') {
            var skuMemberPrice = {};
            skuMemberPrice.memberLevelId = $(this).find('input').attr('levelId');
            skuMemberPrice.price = $(this).find('input').val();
            sku.skuMemberPrices.push(skuMemberPrice)
        }
    });

    $('#edit_vip_price').modal('hide');
}
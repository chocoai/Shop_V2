...
var initDataTable = function () {
    $("#dataTable").lsDataTable({
        "ajax": function (data, callback) {
            LSFetch({
                url: basePath + '/querylogisticscompanys.htm',
...
function savelogistics() {
    ...
    LSFetch({
        url: basePath + '/addlogisticscompany.htm',
...
function deleteLogistics() {
    LSFetch({
        url: basePath + '/deletelogisticscompany.htm?id=' + $("#deleteId").val(),
        success: function (res) {
...
function deleteAll(ids) {
    ...
    LSFetch({
        url: basePath + '/deletelogisticscompanys.htm?ids=' + ids,
        success: function (res) {
            if (res == -1) {
                showerror('物流公司已经被物流模版使用,不能删除!');
            }
...
function toupdate(id) {
    ...
    LSFetch({
        url: basePath + '/querylogisticscompanybyid.htm?id=' + id,
        success: function (res) {
            $("#uname").val(res.name);
            $("#ucode").val(res.code);
...
function updatelogistics() {
    ...
    LSFetch({
        url: basePath + '/updatelogisticscompany.htm',
        data: JSON.stringify(logisticscompany),
        contentType: "application/json;charset=utf-8",
        success: function () {

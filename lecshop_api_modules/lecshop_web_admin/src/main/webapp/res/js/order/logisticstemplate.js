...
function queryTemplates() {
    LSFetch({
        url: basePath + '/queryalllogisticstemplates.htm',
...
function setdefault() {
    LSFetch({
        url: basePath + '/setdefaulttemplate.htm?id=' + $("#defaultId").val(),
...
// 跳转都新增页面
function toadd() {
    window.location.href = basePath + "toaddtemplate.htm";
}
...
function deleteone() {
    LSFetch({
        url: basePath + '/deletelogisticstemplatebyid.htm?id=' + $("#deleteId").val(),
        success: function (res) {
...
function toupdate(id) {
    window.location.href = basePath + "toupdatelogisticstemplate.htm?id=" + id;
}
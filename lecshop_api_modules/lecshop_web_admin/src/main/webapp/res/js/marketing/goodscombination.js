...
var initDataTable = function () {
    $("#dataTable").lsDataTable({
        "ajax": function (data, callback) {
            LSFetch({
                url: basePath + '/querygoodscombination.htm',
        ...
        "columns": [
            ...
            {
                "data": function (row) {
                    var operator = '<div class="operation_box"><button class="btn btn-success btn-xs view_btn" type="button" onclick="toEditGoods(' + row.id + ')"><i class="icon-eye-open"></i> 管理商品</button>';
...
function addGoodsCombination() {
    ...
    LSFetch({
        url: basePath + '/addgoodscombination.htm',
...
function deleteGoodsCombination() {
    LSFetch({
        url: basePath + '/deletegoodscombination.htm?id=' + $("#deleteId").val(),
        success: function () {
...
function deleteAll(ids) {
    LSFetch({
        url: basePath + '/batchdeletegoodscombination.htm',
        data: {
            ids: ids
        },
...
function toupdate(id) {
    clearCount();
    LSFetch({
        url: basePath + '/querygoodscombinationbyid.htm?id=' + id,
        success: function (res) {
...
function update() {
    ...
    LSFetch({
        url: basePath + '/updategoodscombination.htm',
        data: JSON.stringify(goodsCombination),
        contentType: "application/json;charset=utf-8",
        success: function () {
...
function toEditGoods(id) {
    window.location.href = basePath + "tomanagegoodscombination.htm" + window.location.search + "&id=" + id;
}
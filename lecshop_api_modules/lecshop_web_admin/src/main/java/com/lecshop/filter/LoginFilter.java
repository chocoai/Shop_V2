...
public class LoginFilter implements Filter {
     ...
     */
    static {
        noNeedFilters.addAll(Stream.of("/querybaseinfosetunauth.htm","/createkaptcha.htm", "/login.htm", "/lockscanner.htm", "/tologin.htm","/locklogin.htm").collect(Collectors.toList()));
    }
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (!AdminLoginUtils.getInstance().isLogin(request)) {
            try {
                ...
                response.sendRedirect("lockscanner.htm");
            } catch (Exception e) {
                e.printStackTrace();

...
var initDataTable = function () {
    $("#dataTable").lsDataTable({
        "ajax": function (data, callback) {
            LSFetch({
                url: basePath + '/store_querythematic.htm',
...
// 跳转到添加专题页面
function toAddThematic() {
    window.location.href = basePath + 'store_toaddthematic.htm' + window.location.search;
}
...
function deleteThematic() {
    LSFetch({
        url: basePath + '/store_deletethematic.htm?id=' + $("#deleteId").val(),
        success: function () {
...
function deleteAll(ids) {
    LSFetch({
        url: basePath + '/store_batchdeletethematic.htm',
        data: {
            ids: ids
        },
...
function toupdate(id) {
    window.location.href = basePath + 'store_toeditthematic.htm' + window.location.search + "&id=" + id;
}

...
function savefcate() {
    ...
    category.delFlag = "0";
    LSFetch({
        url: basePath + 'store_addspucategory.htm',
...
function toaddscate() {
    ...
    LSFetch({
        url: basePath + 'store_queryspucategorybyparentid.htm?parentId=0',
        success: function (data) {
            var html = '';
            for (var i = 0; i < data.length; i++) {
                var cate = data[i];
...
function toaddtcate() {
    ...
    LSFetch({
        url: basePath + 'store_queryspucategorybyparentid.htm?parentId='+$("#curfcateId").val(),
        success: function (data) {
            var html = '';
            for (var i = 0; i < data.length; i++) {
                var cate = data[i];
...
function deleteCate() {
    LSFetch({
        url: basePath + 'store_deletespucategory.htm?id=' + $('#deleteCateId').val(),
...
function toeditfcate(id) {
    LSFetch({
        url: basePath + 'store_queryspucategorybyid.htm?id=' + id,
        success: function (data) {
...
function toeditscate(id) {
    ...
    LSFetch({
        url: basePath + 'store_queryspucategorybyid.htm.htm?id=' + id,
        success: function (data) {
...
function updatefcate() {
    ...
    LSFetch({
        url: basePath + 'store_updatespucategory.htm',
        data: JSON.stringify(category),
        contentType: "application/json;charset=utf-8",
        success: function () {
            location.reload();

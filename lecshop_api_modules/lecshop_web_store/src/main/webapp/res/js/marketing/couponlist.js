...
var initDataTable = function () {
    $("#dataTable").lsDataTable({
        "ajax": function (data, callback) {
            LSFetch({
                url: basePath + '/store_querycoupon.htm',
...
function copyUrl() {
    ...
    $(".copy").zclip({
        ...
        copy: function () {
            $.ajax({
                url: basePath + '/store_copycoupon.htm',
                data: {
...
function deleteSaveBtn() {
    ...
    LSFetch({
        url: basePath + '/store_deletecoupon.htm',
        data: {
            ids: $("#delId").val()
        },
...
function exportCoupon(id) {
    window.open('store_exportcoupon.htm?couponId=' + id);
}

//查看优惠券详情
function toSeeDetails(id) {
    location.href = basePath + "store_tocoupondetails.htm" + window.location.search + "&cid=" + id;
}
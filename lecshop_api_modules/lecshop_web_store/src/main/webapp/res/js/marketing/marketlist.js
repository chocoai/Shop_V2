...
var initDataTable = function () {
    $("#dataTable").lsDataTable({
        "ajax": function (data, callback) {
            LSFetch({
                url: basePath + '/store_querymarketings.htm',
...
function toupdate(id, type) {
    if (type == "1") {
        // 直降
        window.location.href = basePath + "store_toupdatefallmarketing.htm?id=" + id;
    } else if (type == "2") {
        // 团购
        window.location.href = basePath + "store_toupdategroupmarketing.htm?id=" + id;
    } else if (type == "3") {
        // 抢购
        window.location.href = basePath + "store_toupdatepanicmarketing.htm?id=" + id;
    } else if (type == "4") {
        window.location.href = basePath + "store_toupdatefulldown.htm?id=" + id;
    }else if (type == "5") {
        window.location.href = basePath + "store_toupdatefulldiscount.htm?id=" + id;
    }
}

...
var initSkuDataTable = function () {
    $("#skuDataTable").lsDataTable({
        "ajax": function (data, callback) {
            LSFetch({
                url: basePath + '/store_queryskuofgoodscombination.htm',
...
function loadSkus() {
    ...
    for (var i = 0; i < selectedSkus.length; i++) {
        ...
        LSFetch({
            url: basePath + '/store_addcombinationsku.htm',
...
function deleteSku() {
    LSFetch({
        url: basePath + '/store_deletecombinationsku.htm?id=' + $("#deleteId").val(),
        success: function () {
...
function deleteAll(ids) {
    LSFetch({
        url: basePath + '/store_batchdeletecombinationsku.htm',
        data: {
            ids: ids
        },
